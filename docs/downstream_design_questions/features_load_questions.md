# features/load.py Contract

## Purpose

`load.py` persists **validated feature rows** into the feature table.

It is responsible for writing accepted feature data and handling duplicates.
...
Accepted feature rows are inserted into:
...
Rejected feature rows are NOT persisted in this slice.

This path is explicitly deferred.
...
`load.py`:
- inserts validated feature rows
...
> "How do we persist validated feature rows into the feature table correctly?"
